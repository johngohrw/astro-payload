...
- `labels.singular` and `labels.plural` = human-readable, title-cased.
...
| image URL string | `upload` with `relationTo: 'media'` |
| string union / enum | `select` with options |

- Optional props → no `required: true`.
- Props with defaults → add `defaultValue` matching the Astro default.
...
- **Image URLs** — use `upload` with `relationTo: 'media'`; note the assumption.
