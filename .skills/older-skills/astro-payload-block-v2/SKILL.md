...
- Props with defaults → add `defaultValue` matching the Astro default, **except for `upload` fields and arrays containing `upload` fields** (omit `defaultValue` to avoid Payload CMS errors).
...
Always check the type before accessing properties:

```astro
const imageUrl = typeof image === "object" && image !== null ? image.url : undefined;
```
...
items: items?.map((item) => ({
  imageSrc: typeof item.image === "object" && item.image !== null ? item.image.url : undefined,
  imageAlt: typeof item.image === "object" && item.image !== null ? item.image.alt : undefined,
})),
```

### Remapping rules

- `null` → `undefined`: always use `?? undefined` on nullable fields.
- Grouped fields (e.g. `announcement.text`) → flat props (e.g. `announcementText`).
- Optional groups → ternary guard:
  `groupField ? { label: groupField.label, href: groupField.href } : undefined`
- Arrays → `.map()` over items, spreading only the known fields.
...
- **Image URLs** — always use `upload` with `relationTo: 'media'`; omit `defaultValue` for upload fields and arrays containing upload fields.
- **Media type checking** — in Render.astro, always check `typeof field === "object" && field !== null` before accessing `.url` or other Media properties.
